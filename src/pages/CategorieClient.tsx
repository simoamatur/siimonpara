...
import { Search, Plus, Edit2, Trash2, X, Users, Save, ArrowLeft } from 'lucide-react';
...
export const CategorieClient: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<Categorie>>({ nom: '' });

  const fetchData = async () => {
    try { const res = await axios.get('/api/parametres/categories-clients'); setItems(res.data.data || res.data); }
    catch { /* ignore */ }
  ...
  const handleEdit = (v: Categorie) => { setSelected(v); setFormData({ nom: v.nom }); setShowForm(true); };

  const handleSave = async () => {
    setSubmitting(true);
    try {
      if (selected) await axios.put(`/api/parametres/categories-clients/${selected.id}`, { nom: formData.nom });
      else await axios.post('/api/parametres/categories-clients', { nom: formData.nom });
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer cette catégorie ?'}))) return;
    try { await axios.delete(`/api/parametres/categories-clients/${id}`); await fetchData(); }
    catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Catégories Clients">
      <div className=" mx-auto">
        {!showForm ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Users size={24} /><h2 className="text-lg font-bold">Catégories Clients</h2></div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Users size={24} /><h2 className="text-lg font-bold">{selected ? 'Modifier' : 'Nouvelle Catégorie'}</h2></div>
