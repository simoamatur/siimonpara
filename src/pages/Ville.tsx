...
import { Search, Plus, Edit2, Trash2, X, Building2, Save, ArrowLeft } from 'lucide-react';
...
export const Ville: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<Ville>>({ nom: '' });
  ...
  const filtered = villes.filter(v => v.nom.toLowerCase().includes(searchQuery.toLowerCase()));

  const handleAdd = () => { setSelectedVille(null); setFormData({ nom: '' }); setShowForm(true); };
  const handleEdit = (v: Ville) => { setSelectedVille(v); setFormData({ nom: v.nom }); setShowForm(true); };

  const handleSave = async () => {
    setSubmitting(true);
    try {
      if (selectedVille) await axios.put(`/api/parametres/villes/${selectedVille.id}`, { nom: formData.nom });
      else await axios.post('/api/parametres/villes', { nom: formData.nom });
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer cette ville ?'}))) return;
    try { await axios.delete(`/api/parametres/villes/${id}`); await fetchData(); }
    catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Villes">
      <div className=" mx-auto">
        {!showForm ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Building2 size={24} /><h2 className="text-lg font-bold">Villes</h2></div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-6">
                <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Rechercher ville..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full" /></div>
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Building2 size={24} /><h2 className="text-lg font-bold">{selectedVille ? 'Modifier Ville' : 'Nouvelle Ville'}</h2></div>
