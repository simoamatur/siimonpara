...
export const TVA: React.FC = () => {
  const confirm = useConfirm();
  const [searchQuery, setSearchQuery] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [selectedTVA, setSelectedTVA] = useState<TVAItem | null>(null);
  const [items, setItems] = useState<TVAItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);

  const [formData, setFormData] = useState<Partial<TVAItem>>({ taux: 20, libelle: '' });

  const fetchData = async () => {
    try { const res = await axios.get('/api/parametres/tva'); setItems(res.data.data || res.data); }
    catch { /* ignore */ }
  ...
  const filtered = items.filter(i =>
    i.libelle.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const handleAdd = () => { setSelectedTVA(null); setFormData({ taux: 20, libelle: '' }); setShowForm(true); };
  const handleEdit = (v: TVAItem) => { setSelectedTVA(v); setFormData({ taux: v.taux, libelle: v.libelle }); setShowForm(true); };

  const handleSave = async () => {
    setSubmitting(true);
    try {
      if (selectedTVA) await axios.put(`/api/parametres/tva/${selectedTVA.id}`, { taux: formData.taux, libelle: formData.libelle });
      else await axios.post('/api/parametres/tva', { taux: formData.taux, libelle: formData.libelle });
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer ce taux de TVA ?'}))) return;
    try { await axios.delete(`/api/parametres/tva/${id}`); await fetchData(); }
    catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Gestion des TVA">
      <div className=" mx-auto">
        {!showForm ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Percent size={24} /><h2 className="text-lg font-bold">Taux de TVA</h2></div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-6">
                <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Rechercher TVA..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full" /></div>
              </div>
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200">
                <table className="w-full">
                  <thead className="bg-gray-100"><tr><th className="px-4 py-3 text-left text-xs font-bold text-gray-600">Libellé</th><th className="px-4 py-3 text-center text-xs font-bold text-gray-600">Taux</th><th className="px-4 py-3 text-center text-xs font-bold text-gray-600">Action</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {filtered.map((v) => (
                      <tr key={v.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm text-gray-600">{v.libelle}</td>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Percent size={24} /><h2 className="text-lg font-bold">{selectedTVA ? 'Modifier TVA' : 'Nouveau Taux TVA'}</h2></div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div className="space-y-2"><label className="text-sm font-bold text-gray-600">Libellé:</label><input type="text" value={formData.libelle} onChange={(e) => setFormData({ ...formData, libelle: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></div>
