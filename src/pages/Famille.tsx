...
import { Search, Plus, Edit2, Trash2, X, Folder, Save, ArrowLeft } from 'lucide-react';
...
export const Famille: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<Famille>>({ nom: '' });

  const fetchFamilles = async () => {
    try {
      const res = await axios.get('/api/parametres/familles');
      setFamilles(res.data.data || res.data);
    } catch { /* ignore */ }
  ...
  const handleSave = async () => {
    ...
    try {
      if (selectedFamille) {
        await axios.put(`/api/parametres/familles/${selectedFamille.id}`, { nom: formData.nom });
      } else {
        await axios.post('/api/parametres/familles', { nom: formData.nom });
      }
      await fetchFamilles();
      setShowForm(false);
    } catch { /* ignore */ }
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer cette famille ?'}))) return;
    try {
      await axios.delete(`/api/parametres/familles/${id}`);
      await fetchFamilles();
    } catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Familles">
      <div className=" mx-auto">
        {!showForm ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Folder size={24} /><h2 className="text-lg font-bold">Familles d'Articles</h2></div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-6">
                <div className="relative flex-1">
                  ...
                  <input type="text" placeholder="Rechercher famille..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full" />
                </div>
              </div>
              {loading ? (
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredFamilles.map((f) => (
                      <tr key={f.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm text-gray-600">{f.nom}</td>
                        <td className="px-4 py-3 text-center"><div className="flex items-center justify-center gap-1"><button onClick={() => handleEdit(f)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Edit2 size={16} /></button><button onClick={() => handleDelete(f.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Folder size={24} /><h2 className="text-lg font-bold">{selectedFamille ? 'Modifier Famille' : 'Nouvelle Famille'}</h2></div>
