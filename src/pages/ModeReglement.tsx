...
import { Search, Plus, Edit2, Trash2, X, CreditCard, Save, ArrowLeft } from 'lucide-react';
...
export const ModeReglement: React.FC = () => {
  const confirm = useConfirm();
  const [searchQuery, setSearchQuery] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [selected, setSelected] = useState<ModeReglement | null>(null);
  const [items, setItems] = useState<ModeReglement[]>([]);
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);

  const [formData, setFormData] = useState<Partial<ModeReglement>>({ nom: '' });

  const fetchData = async () => {
    try { const res = await axios.get('/api/parametres/modes-reglement'); setItems(res.data.data || res.data); }
    catch { /* ignore */ }
  ...
  const filtered = items.filter(i => i.nom.toLowerCase().includes(searchQuery.toLowerCase()));

  const handleAdd = () => { setSelected(null); setFormData({ nom: '' }); setShowForm(true); };
  const handleEdit = (v: ModeReglement) => { setSelected(v); setFormData({ nom: v.nom }); setShowForm(true); };

  const handleSave = async () => {
    setSubmitting(true);
    try {
      if (selected) await axios.put(`/api/parametres/modes-reglement/${selected.id}`, { nom: formData.nom });
      else await axios.post('/api/parametres/modes-reglement', { nom: formData.nom });
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer ce mode de règlement ?'}))) return;
    try { await axios.delete(`/api/parametres/modes-reglement/${id}`); await fetchData(); }
    catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Modes de Règlement">
      <div className=" mx-auto">
        {!showForm ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><CreditCard size={24} /><h2 className="text-lg font-bold">Modes de Règlement</h2></div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-6">
                <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Rechercher..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full" /></div>
              </div>
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200">
                <table className="w-full">
                  <thead className="bg-gray-100"><tr><th className="px-4 py-3 text-left text-xs font-bold text-gray-600">Nom</th><th className="px-4 py-3 text-center text-xs font-bold text-gray-600">Action</th></tr></thead>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><CreditCard size={24} /><h2 className="text-lg font-bold">{selected ? 'Modifier' : 'Nouveau Mode de Règlement'}</h2></div>
