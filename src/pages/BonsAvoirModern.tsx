...
import {
  Plus, Search, Receipt, Eye, Trash2,
  CheckCircle2, Clock, ArrowUpDown
} from 'lucide-react';
...
export const BonsAvoirModern: React.FC = () => {
  const navigate = useNavigate();
  const confirm = useConfirm();
  const [items, setItems] = useState<BonAvoir[]>([]);
  ...
  const fetchData = async () => {
    try { const res = await axios.get('/api/ventes/avoirs'); setItems(res.data.data || res.data); }
    catch { /* ignore */ }
  ...
  const stats = useMemo(() => ({
    total: items.length,
    used: items.filter(i => i.utilise).length,
    totalAmount: items.reduce((a, i) => a + i.totalTTC, 0),
  }), [items]);
  ...
  const filtered = useMemo(() => {
    ...
    r.sort((a, b) => { let c = 0; if (sortBy === 'date') c = new Date(a.date).getTime() - new Date(b.date).getTime(); else c = a.reference.localeCompare(b.reference); return sortOrder === 'asc' ? c : -c; });
    return r;
  }, [items, searchQuery, sortBy, sortOrder]);

  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer cet avoir ?'}))) return;
    try { await axios.delete(`/api/ventes/avoirs/${id}`); await fetchData(); }
    catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Bons d'Avoir">
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { label: 'Total Avoirs', value: stats.total, icon: Receipt, color: 'from-blue-500 to-cyan-500' },
            { label: 'Utilisés', value: stats.used, icon: CheckCircle2, color: 'from-emerald-500 to-teal-500' },
            { label: 'Montant Total', value: `${stats.totalAmount.toLocaleString('fr-FR')} DH`, icon: Receipt, color: 'from-violet-500 to-purple-500' },
          ].map((s, i) => (
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              ...
              <input type="text" placeholder="Rechercher avoir..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
          ...
          <button onClick={() => navigate('/dashboard/vente/avoirs/nouveau')}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((item, i) => (
                  <motion.tr key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <td className="px-3 py-3 text-right"><span className="text-sm font-bold text-gray-700">{item.totalTTC.toLocaleString('fr-FR', { minimumFractionDigits: 2 })} DH</span></td>
                    <td className="px-3 py-3 text-center">
                      {item.utilise
                        ? <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold border bg-emerald-50 text-emerald-600 border-emerald-200"><CheckCircle2 size={14} />Utilisé</span>
                        : <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold border bg-blue-50 text-blue-600 border-blue-200"><Clock size={14} />Disponible</span>
                      }
                    </td>
                    <td className="px-3 py-3 text-center">
          ...
          {filtered.length === 0 && !loading && (
            <div className="p-12 text-center"><div className="w-16 h-16 mx-auto mb-4 rounded-xl bg-gray-100 flex items-center justify-center"><Receipt size={32} className="text-slate-400" /></div><p className="text-gray-400 font-medium">Aucun avoir trouvé</p></div>
          )}
