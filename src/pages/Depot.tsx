...
import { Search, Plus, Edit2, Trash2, X, Warehouse, Save, ArrowLeft } from 'lucide-react';
...
export const Depot: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<Depot>>({ nom: '', adresse: '' });

  const fetchData = async () => {
    try { const res = await axios.get('/api/parametres/depots'); setItems(res.data.data || res.data); }
    catch { /* ignore */ }
  ...
  const handleAdd = () => { setSelectedDepot(null); setFormData({ nom: '', adresse: '' }); setShowForm(true); };
  const handleEdit = (v: Depot) => { setSelectedDepot(v); setFormData({ nom: v.nom, adresse: v.adresse || '' }); setShowForm(true); };

  const handleSave = async () => {
    setSubmitting(true);
    try {
      if (selectedDepot) await axios.put(`/api/parametres/depots/${selectedDepot.id}`, formData);
      else await axios.post('/api/parametres/depots', formData);
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer ce dépôt ?'}))) return;
    try { await axios.delete(`/api/parametres/depots/${id}`); await fetchData(); }
    catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Dépôts">
      <div className=" mx-auto">
        {!showForm ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Warehouse size={24} /><h2 className="text-lg font-bold">Dépôts</h2></div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-6">
                <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Rechercher dépôt..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full" /></div>
              </div>
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200">
                <table className="w-full">
                  <thead className="bg-gray-100"><tr><th className="px-4 py-3 text-left text-xs font-bold text-gray-600">Nom</th><th className="px-4 py-3 text-left text-xs font-bold text-gray-600">Adresse</th><th className="px-4 py-3 text-center text-xs font-bold text-gray-600">Action</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {filtered.map((v) => (
                      <tr key={v.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm font-medium text-gray-700">{v.nom}</td>
                        <td className="px-4 py-3 text-sm text-gray-500">{v.adresse || '-'}</td>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><Warehouse size={24} /><h2 className="text-lg font-bold">{selectedDepot ? 'Modifier Dépôt' : 'Nouveau Dépôt'}</h2></div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-6 mb-6">
                ...
                <div className="space-y-2"><label className="text-sm font-bold text-gray-600">Adresse:</label><input type="text" value={formData.adresse || ''} onChange={(e) => setFormData({ ...formData, adresse: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /></div>
              </div>
