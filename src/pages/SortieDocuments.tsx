...
export const SortieDocuments: React.FC = () => {
      ...
      try {
        ...
        const items = routes.flatMap((r: any) =>
          (r.affectations || []).map((a: any) => ({
            ...
            sortie: a.statut === 'en_cours' || a.statut === 'livré',
            dateSortie: r.date?.split('T')[0] || '',
            livreur: r.livreur?.name || '',
          }))
        );
        setDocs(items.length ? items : []);
      } catch (err) { console.error('Erreur chargement sortie documents:', err); } finally { setLoading(false); }
    };
    fetchData();
  }, [token]);
  ...
  return (
    <DashboardLayout title="Sortie des Documents">
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  filtered.map((d) => (
                    <tr key={d.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-sm text-center text-gray-500">{d.dateSortie}</td>
                      <td className="px-3 py-3 text-sm text-center text-gray-600">{d.livreur}</td>
                      <td className="px-3 py-3 text-center">
                        <span className={`px-2 py-1 rounded-full text-xs font-bold ${d.sortie ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                          {d.sortie ? 'Sorti' : 'En attente'}
                        </span>
                      </td>
                    </tr>
                  ))}
