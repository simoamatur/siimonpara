...
import { Search, Plus, Edit2, Trash2, X, FolderOpen, Save, ArrowLeft } from 'lucide-react';
...
export const SousFamille: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<SousFamille>>({ nom: '', familleId: '' });

  const fetchData = async () => {
    try { const res = await axios.get('/api/parametres/sous-familles'); setItems(res.data.data || res.data); }
    catch { /* ignore */ }
    finally { setLoading(false); }
  };

  const fetchFamilles = async () => {
    try { const res = await axios.get('/api/parametres/familles'); setFamilles(res.data.data || res.data); }
    catch { /* ignore */ }
  };

  useEffect(() => { fetchData(); fetchFamilles(); }, []);

  const filtered = items.filter(i =>
    i.nom.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const handleAdd = () => { setSelectedSousFamille(null); setFormData({ nom: '', familleId: '' }); setShowForm(true); };
  const handleEdit = (v: SousFamille) => { setSelectedSousFamille(v); setFormData({ nom: v.nom, familleId: v.familleId }); setShowForm(true); };

  const handleSave = async () => {
    setSubmitting(true);
    try {
      if (selectedSousFamille) await axios.put(`/api/parametres/sous-familles/${selectedSousFamille.id}`, formData);
      else await axios.post('/api/parametres/sous-familles', formData);
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer cette sous-famille ?'}))) return;
    try { await axios.delete(`/api/parametres/sous-familles/${id}`); await fetchData(); }
    catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Sous-Familles">
      <div className=" mx-auto">
        {!showForm ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><FolderOpen size={24} /><h2 className="text-lg font-bold">Sous-Familles</h2></div>
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-6">
                <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Rechercher sous-famille..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full" /></div>
              </div>
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200">
                <table className="w-full">
                  <thead className="bg-gray-100"><tr><th className="px-4 py-3 text-left text-xs font-bold text-gray-600">Nom</th><th className="px-4 py-3 text-left text-xs font-bold text-gray-600">Famille</th><th className="px-4 py-3 text-center text-xs font-bold text-gray-600">Action</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {filtered.map((v) => (
                      <tr key={v.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm text-gray-600">{v.nom}</td>
                        <td className="px-4 py-3 text-sm text-gray-500">{v.famille?.nom || '-'}</td>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 text-white px-6 py-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3"><FolderOpen size={24} /><h2 className="text-lg font-bold">{selectedSousFamille ? 'Modifier Sous-Famille' : 'Nouvelle Sous-Famille'}</h2></div>
