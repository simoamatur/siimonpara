...
import {
  Plus, Search, RotateCcw, Eye, Trash2,
  CheckCircle2, Clock, AlertCircle, Calendar, ArrowUpDown
} from 'lucide-react';
...
export const BonsRetourModern: React.FC = () => {
  const navigate = useNavigate();
  const confirm = useConfirm();
  const [items, setItems] = useState<BonRetour[]>([]);
  ...
  const fetchData = async () => {
    try { const res = await axios.get('/api/ventes/retours'); setItems(res.data.data || res.data); }
    catch { /* ignore */ }
  ...
  const handleDelete = async (id: string) => {
    if (!(await confirm({ message: 'Supprimer ce retour ?'}))) return;
    try { await axios.delete(`/api/ventes/retours/${id}`); await fetchData(); }
    catch { /* ignore */ }
  };

  return (
    <DashboardLayout title="Bons de Retour">
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { label: 'Total Retours', value: stats.total, icon: RotateCcw, color: 'from-blue-500 to-cyan-500' },
            { label: 'Montant Total', value: `${stats.totalAmount.toLocaleString('fr-FR')} DH`, icon: AlertCircle, color: 'from-amber-500 to-orange-500' },
          ].map((s, i) => (
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              ...
              <input type="text" placeholder="Rechercher retour..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
          ...
          <button onClick={() => navigate('/dashboard/vente/retours/nouveau')}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-400 uppercase">Motif</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((item, i) => (
                  <motion.tr key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <td className="px-3 py-3 text-sm text-gray-500">{item.motif || '-'}</td>
          ...
          {filtered.length === 0 && !loading && (
            <div className="p-12 text-center"><div className="w-16 h-16 mx-auto mb-4 rounded-xl bg-gray-100 flex items-center justify-center"><RotateCcw size={32} className="text-slate-400" /></div><p className="text-gray-400 font-medium">Aucun retour trouvé</p></div>
          )}
